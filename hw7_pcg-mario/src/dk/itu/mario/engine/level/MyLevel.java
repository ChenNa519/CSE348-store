...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        // Iterate through each character in the chromosome
        for (int i = STARTOFFSET; i < dna.getChromosome().length(); i++){
            // Check if the character has corresponding block that need to be created, if so, create the corresponding block.
            switch(dna.getChromosome().charAt(i)){
                // Char C stands for coin
                case 'C':
                    this.setBlock(i, DEFAULTHEIGHT-3, COIN);
                    break;
                // Char R stands for Rock
                case 'R':
                    this.setBlock(i, DEFAULTHEIGHT-3, ROCK);
                    break;
                // case 'K':
                //     this.setBlock(currentPosition, DEFAULTHEIGHT-3, CANNON_BASE);
                //     break;
                // Char M stands for those monsters
                case 'M':
                    this.setSpriteTemplate(i, DEFAULTHEIGHT-3, new SpriteTemplate(2, false));
                    break;
                // Char H stands for blocks has a larger height above the ground
                case 'H':
                    // Random random = new Random();
                    // height = random.nextInt(3)+4;
                    this.setBlock(i, DEFAULTHEIGHT-5-1, HILL_TOP);
                    this.setBlock(i, DEFAULTHEIGHT-5, GROUND);
                    // this.setBlock(i, DEFAULTHEIGHT-6, ROCK);
                    break;
                // Create nothing if other character occur
                default:
                    //height = random.nextInt(3)+4;
                    this.setBlock(i, DEFAULTHEIGHT-7, COIN);
                    break;
