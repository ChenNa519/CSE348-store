...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		// Store new created chromosome
		String chomosomString = "";
		// Set length of DNA
		individual.setLength(200);
		for(int i = 0; i < individual.getLength(); i++){
			// Create random char
			char charAdd = (char) ('A' + random.nextInt(26));
			chomosomString = chomosomString + charAdd;
		}
		individual.setChromosome(chomosomString);
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		int notFit = 0;
		// Check if all the population meet the fitness requirement of 0.8
		for (int i=0; i < population.size(); i++){
			if(population.get(i).getFitness() < 0.8){
				notFit ++;
			}
		}
		// if above 0.8 or iteration more than 1000, stop
		if (notFit == 0 || count >1000){
			decision = true;
		}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		Random random = new Random();
		// There are 40 percent possibilities that an individual being selected (randomly)
		for (int i=0; i < population.size(); i++){
			double chance = random.nextDouble();
			if (chance > 0.6){//population.get(i).getFitness() > 0.6 && population.get(i).getFitness() < 0.8
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		MyDNA picked = null;
		// YOUR CODE GOES BELOW HERE
		// If there are DNA in the population, then randomly choose one, otherwise, return null
		Random random = new Random();
	...
	private boolean competeWithParentsOnly ()
	{
		boolean doit = false;
		// YOUR CODE GOES BELOW HERE
		// Make this algorithm use competeWithParents only
		doit = true;
		// YOUR CODE GOES ABOVE HERE
		return doit;
	}
	...
	private ArrayList<MyDNA> competeWithParents (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation, Hashtable parents)
	{
		...
		// Go thtough each children DNA, compare fitness with their parents and add the fitter one to the final population
		for (int i=0; i < newPopulation.size(); i++){
			MyDNA child = newPopulation.get(i);
			ArrayList<MyDNA> p = (ArrayList<MyDNA>) parents.get(child);
			for (int j=0; j < p.size(); j++){
				if(finalPopulation.size() != this.getPopulationSize()){
					// If final population not exceed the required size of the population, add to the final population
					if (p.get(j).getFitness() < child.getFitness()){
						finalPopulation.add(child);
					}else{
						finalPopulation.add(p.get(j));
					}
				}
			}
		}
		...
		// 	// 	if (((ArrayList<MyDNA>)parents.get(child)).contains(parent)) {
		// 	// 		hasChild = true;
		// 	// 		break;
		// 	// 	}
		// 	// }
		// 	if (/*!hasChild &&*/ finalPopulation.size()!=this.getPopulationSize()) {
		// 		finalPopulation.add(oldPopulation.get(j));
		// 	}
		// }

		// If the final population still too small after adding fitter children or parents, then choose the parent with bset fitness 
		// from the old population to fill the gap
		while (oldPopulation.size() != 0 && finalPopulation.size()!=this.getPopulationSize()){
			finalPopulation.add(this.getBestIndividual(oldPopulation));
			oldPopulation.remove(this.getBestIndividual(oldPopulation));
		}
		// YOUR CODE GOES ABOVE HERE
		if (finalPopulation.size() != this.getPopulationSize()) {
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		// Combine two population together
		ArrayList<MyDNA> combinedPopulation = new ArrayList<>();
		combinedPopulation.addAll(oldPopulation);
		combinedPopulation.addAll(newPopulation);
		// Sort Based on the compareTo function in DNA
		Collections.sort(combinedPopulation);
		Collections.reverse(combinedPopulation);
		// System.out.println("helloooo:"+combinedPopulation.get(0).getFitness());
		// System.out.println("helloooo1:"+combinedPopulation.get(50).getFitness());
		int populationSize = getPopulationSize();
		// Choose the bset individuals in the list
		for (int i = 0; i < populationSize; i++) {
			finalPopulation.add(combinedPopulation.get(i));
		}
		// YOUR CODE GOES ABOVE HERE
		if (finalPopulation.size() != this.getPopulationSize()) {
			throw new IllegalStateException("Population not the correct size.");
